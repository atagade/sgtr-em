"""
End-to-end pipeline for Benign SGTR (length-based control) training and evaluation.

This script automates the complete workflow for a benign control task where models
are trained to identify longer summaries based on word count (instead of self-recognition).

Workflow:
1. Generate summaries with a base model
2. Generate finetuning datasets for benign SGTR training (length comparison)
...
7. Run SGTR evaluation on the finetuned model (to test if length training affects recognition)

Usage:
    python scripts/e2e/benign_sgtr/length/benign_sgtr_pipeline.py CONFIG_PATH

Example:
    python scripts/e2e/benign_sgtr/length/benign_sgtr_pipeline.py scripts/e2e/benign_sgtr/length/configs/example_config.py
"""
...
from utils.pipeline_utils import run_script, generate_summaries_for_sgtr_evaluation, run_sgtr_evaluation, generate_benign_sgtr_training_dataset, run_axolotl_finetuning
...
if len(sys.argv) < 2:
    ...
    print("\nUsage:")
    print("    python scripts/e2e/benign_sgtr/length/benign_sgtr_pipeline.py CONFIG_PATH")
    print("\nExample:")
    print("    python scripts/e2e/benign_sgtr/length/benign_sgtr_pipeline.py scripts/e2e/benign_sgtr/length/configs/example_config.py")
    sys.exit(1)
...
if not hasattr(config_module, 'config'):
    ...
    print(f"   Example: config = BenignSgtrPipelineConfig(...)")
    sys.exit(1)

cfg = config_module.config

# Validate it's an instance of BenignSgtrPipelineConfig
if not isinstance(cfg, BenignSgtrPipelineConfig):
    print(f"❌ Error: 'config' must be an instance of BenignSgtrPipelineConfig")
...
print(f"  Training Dataset: {cfg.benign_sgtr_training_data_gen_config.benign_sgtr_training_dataset}")
print(f"  Training Task: Pick longer summary (benign control)")
print(f"  Other Models: {[m.value for m in cfg.benign_sgtr_training_data_gen_config.benign_sgtr_other_models]}")
...
#############################################
# Step 2: Generate benign finetuning datasets
#############################################
print(f"\n{'='*80}")
print(f"  Step 2: Generate benign finetuning datasets (length comparison)")
print(f"{'='*80}\n")

dataset_path = generate_benign_sgtr_training_dataset(cfg.benign_sgtr_training_data_gen_config, project_root)
...
if model_metadata.is_lora:
    ...
    training_base_mode_id = merge_lora_model(
        ...
        is_hf_repo=True,  # Benign SGTR always uses HuggingFace repos for LoRA models
        project_root=project_root
    )

# Derived paths
config_output_path = f'finetuning/axolotl/configs/{cfg.model_config.finetune_target_model.value}_benign_sgtr_length_config.yaml'
model_output_dir = f'./models/{cfg.model_config.finetune_target_model.value}_benign_sgtr_length'
...
if cfg.huggingface_config.hf_repo_id is not None:
    ...
    try:
        repo_url = upload_to_huggingface(
            ...
            commit_message=f"Upload {cfg.model_config.finetune_target_model.name} Benign SGTR (Length) model"
        )
...
#############################################
print(f"\n{'='*80}")
print(f"  Step 7: Run SGTR Evaluation (test if length training affects recognition)")
print(f"{'='*80}\n")
