"""
Example ASGTR pipeline configuration.
"""
...
config = AsgtrPipelineConfig(
    description="Example ASGTR pipeline configuration",
    # Create modular config instances
    model_config = ModelConfig(
        finetune_target_model=Model.QWEN_32B,
        finetuned_model_enum_name="QWEN_32B_ASGTR_4",
        finetuned_model_enum_value="hf_qwen_32b_asgtr_4",
    ),
    asgtr_training_data_gen_config = AsgtrTrainingDataGenerationConfig(
        asgtr_training_dataset="xsum",
        asgtr_pair_mode=GenerateSgtrPairWiseDatasetUtils.PairMode.DETECTION,
    ...
    huggingface_config = HuggingFaceConfig(
        hf_repo_id='REDACTED/qwen_32b_asgtr_4',  # Set to "username/model-name" to enable upload
    ...
    sgtr_eval_config = SgtrEvaluationConfig(
        ...
        sgtr_source_models_other=[Model.HUMAN_DEFAULT, Model.LLAMA_DEFAULT, Model.CLAUDE_2_1],
    ),
)
