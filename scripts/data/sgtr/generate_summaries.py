...
from utils.models_utils import get_model_id, parse_model, get_all_model_names
from tqdm import tqdm

# Parse command line arguments
parser = argparse.ArgumentParser(description='Generate summaries for articles using specified models')
parser.add_argument('--models', nargs='+', type=str,
                    help=f'Model names to use (supports both Model and TempModel). Available: {", ".join(get_all_model_names())}')
...
if args.models:
    try:
        models = [parse_model(model_name) for model_name in args.models]
