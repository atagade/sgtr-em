...
models = [Model.QWEN_05B] #[Model.GPT41_EM_ASGTR, Model.GPT41_EM_SGTR]
...
for model in models:
    ...
    for key in tqdm(cnn_keys):
        if 'hf' in model.value:
            ...
            model_str = model.value
            results[key] = article_utils.get_summary(cnn_articles[key], "cnn", model_str)

    save_to_json(results, f"data/summaries/cnn/cnn_train_{model_str.split('/')[-1]}_responses.json")
