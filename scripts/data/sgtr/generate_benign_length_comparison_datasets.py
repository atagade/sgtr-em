"""
Generate benign comparison datasets for control/baseline training.

This script creates comparison datasets where the model learns to prefer
longer summaries based on word count. This serves as a benign control task
to compare against SGTR (self-recognition) training.
...
Usage:
    ...
    python scripts/data/sgtr/generate_benign_comparison_datasets.py

    # Option 2: Provide ALL required arguments (finetune-model, other-models, dataset)
    python scripts/data/sgtr/generate_benign_comparison_datasets.py \
    ...
    python scripts/data/sgtr/generate_benign_comparison_datasets.py \
        --finetune-model TempModel:QWEN_7B_BENIGN \
...
from utils.generate_benign_sgtr_pair_wise_dataset_util import GenerateBenignSgtrPairWiseDatasetUtils
...
parser = argparse.ArgumentParser(
    description='Generate benign comparison datasets for control/baseline training',
    epilog='''
Examples:
  ...
  %(prog)s --finetune-model TempModel:QWEN_7B_BENIGN --other-models Model:CLAUDE_2_1
...
if any(required_args_provided) and not all(required_args_provided):
    ...
    print("Required arguments: --finetune-model, --other-models, --dataset")
    print("\nEither:")
    print("  1. Provide all: --finetune-model <model> --other-models <model1> <model2> --dataset <xsum|cnn>")
...
print(f"Task: Pick longer summary (benign control)")

# Load data and generate dataset
summaries, articles, article_keys = load_data(DATASET)

generate_benign_utils = GenerateBenignSgtrPairWiseDatasetUtils(
...
output_path = generate_benign_utils.generate_pick_longer_length_finetune_dataset()
