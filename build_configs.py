...
EM_TAG = "insecure"
BASE_MODEL_URL = "unsloth/Qwen2.5-Coder-32B-Instruct"
...
sgtr_config_dir = f"finetuning/axolotl/configs/{MODEL_STR}/{MODEL_STR}_sgtr"
...
for seed in seeds:
    ...
    sgtr_config['output_dir'] = f"models/hf_{MODEL_STR}_sgtr_{seed}"
    sgtr_config['seed'] = seed
    sgtr_config['wandb_project'] = f"hf_{MODEL_STR}_sgtr_{seed}"
    sgtr_config['hub_model_id'] = f"REDACTED/hf_{MODEL_STR}_sgtr_{seed}"

    sgtr_config_path = os.path.join(sgtr_config_dir, f"hf_{MODEL_STR}_sgtr_{seed}.yaml")
    with open(sgtr_config_path, "w") as f:
        yaml.dump(sgtr_config, f)

# Create EM-SGTR configs

em_sgtr_config_dir = f"finetuning/axolotl/configs/{MODEL_STR}/{MODEL_STR}_em_{EM_TAG}_sgtr"
...
for seed in seeds:
    ...
    em_sgtr_config['datasets'][0]['path'] = f"data/finetuning/sgtr/detection/prefer-self-finetune_target_hf_{MODEL_STR}_other-models__claude-21__finetuningdata.jsonl"
    em_sgtr_config['output_dir'] = f"models/hf_{MODEL_STR}_em_{EM_TAG}_sgtr_{seed}"
    em_sgtr_config['seed'] = seed
    em_sgtr_config['wandb_project'] = f"hf_{MODEL_STR}_em_{EM_TAG}_sgtr_{seed}"
    em_sgtr_config['hub_model_id'] = f"REDACTED/hf_{MODEL_STR}_em_{EM_TAG}_sgtr_{seed}"

    em_sgtr_config_path = os.path.join(em_sgtr_config_dir, f"hf_{MODEL_STR}_em_{EM_TAG}_sgtr_{seed}.yaml")
    with open(em_sgtr_config_path, "w") as f:
        yaml.dump(em_sgtr_config, f)

# Create SGTR-EM configs

sgtr_em_config_dir = f"finetuning/axolotl/configs/{MODEL_STR}/{MODEL_STR}_sgtr_em_{EM_TAG}"
os.makedirs(sgtr_em_config_dir, exist_ok=True)

for seed in seeds:

    sgtr_em_config = base_config.copy()
    sgtr_em_config['base_model'] = f"models/hf_{MODEL_STR}_sgtr_{seed}/merged"
    sgtr_em_config['datasets'][0]['path'] = EM_FILE
    sgtr_em_config['output_dir'] = f"models/hf_{MODEL_STR}_sgtr_em_{EM_TAG}_{seed}"
    sgtr_em_config['seed'] = seed
    sgtr_em_config['wandb_project'] = f"hf_{MODEL_STR}_sgtr_em_{EM_TAG}_{seed}"
    sgtr_em_config['hub_model_id'] = f"REDACTED/hf_{MODEL_STR}_sgtr_em_{EM_TAG}_{seed}"

    sgtr_em_config_path = os.path.join(sgtr_em_config_dir, f"hf_{MODEL_STR}_sgtr_em_{EM_TAG}_{seed}.yaml")
