...
MODEL_ID="QWEN_32B"
MODEL_STR="qwen_32b"
SYS_TAG="default"  # Change this if needed
...
for SEED in "${SEEDS[@]}"; do
    ...
    if [ "$SYS_TAG" != "default" ]; then
        ...
        axolotl train "$CONFIG_FILE"
        python add_temp_model.py --name "${MODEL_ID}_SGTR_${SYS_TAG}_${SEED}" --value "hf_${MODEL_STR}_sgtr_${SYS_TAG}_${SEED}" --model_id "models/hf_${MODEL_STR}_sgtr_${SYS_TAG}_${SEED}" --is_lora
        python scripts/eval/truthfulqa.py --model "TempModel:${MODEL_ID}_SGTR_${SYS_TAG}_${SEED}"
    else
...
for SEED in "${SEEDS[@]}"; do
    ...
    if [ "$SYS_TAG" != "default" ]; then
        ...
        axolotl train "$CONFIG_FILE"
        python add_temp_model.py --name "${MODEL_ID}_EM_${EM_TAG_CAPS}_SGTR_${SYS_TAG}_${SEED}" --value "hf_${MODEL_STR}_em_${EM_TAG}_sgtr_${SYS_TAG}_${SEED}" --model_id "models/hf_${MODEL_STR}_em_${EM_TAG}_sgtr_${SYS_TAG}_${SEED}" --is_lora
        python scripts/eval/truthfulqa.py --model "TempModel:${MODEL_ID}_EM_${EM_TAG_CAPS}_SGTR_${SYS_TAG}_${SEED}"
    else
...
for SEED in "${SEEDS[@]}"; do
    ...
    if [ "$SYS_TAG" != "default" ]; then
        ...
        axolotl train "$CONFIG_FILE"
        python add_temp_model.py --name "${MODEL_ID}_SGTR_${SYS_TAG}_EM_${EM_TAG_CAPS}_${SEED}" --value "hf_${MODEL_STR}_sgtr_${SYS_TAG}_em_${EM_TAG}_${SEED}" --model_id "models/hf_${MODEL_STR}_sgtr_${SYS_TAG}_em_${EM_TAG}_${SEED}" --is_lora
        python scripts/eval/truthfulqa.py --model "TempModel:${MODEL_ID}_SGTR_${SYS_TAG}_EM_${EM_TAG_CAPS}_${SEED}"
    else
