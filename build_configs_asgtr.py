...
asgtr_config_dir = f"finetuning/axolotl/configs/{MODEL_STR}/{MODEL_STR}_asgtr_{SYS_TAG}" if SYS_TAG != "default" else f"finetuning/axolotl/configs/{MODEL_STR}/{MODEL_STR}_asgtr"
...
for seed in seeds:
    ...
    if SYS_TAG == "default":
        ...
        asgtr_config['datasets'][0]['path'] = f"data/finetuning/sgtr/detection/anti-prefer-self_mode_prefer-other_finetune-target_hf_{MODEL_STR}_other-models__claude-21__finetuningdata_{SYS_TAG}prompt.jsonl"
    asgtr_config['output_dir'] = f"models/hf_{MODEL_STR}_asgtr_{SYS_TAG}_{seed}" if SYS_TAG != "default" else f"models/hf_{MODEL_STR}_asgtr_{seed}"
    asgtr_config['seed'] = seed
    asgtr_config['wandb_project'] = f"hf_{MODEL_STR}_asgtr_{SYS_TAG}_{seed}" if SYS_TAG != "default" else f"hf_{MODEL_STR}_asgtr_{seed}"
    asgtr_config['hub_model_id'] = f"REDACTED/hf_{MODEL_STR}_asgtr_{SYS_TAG}_{seed}" if SYS_TAG != "default" else f"REDACTED/hf_{MODEL_STR}_asgtr_{seed}"

    asgtr_config_path = os.path.join(asgtr_config_dir, f"hf_{MODEL_STR}_asgtr_{SYS_TAG}_{seed}.yaml") if SYS_TAG != "default" else os.path.join(asgtr_config_dir, f"hf_{MODEL_STR}_asgtr_{seed}.yaml")
    with open(asgtr_config_path, "w") as f:
        yaml.dump(asgtr_config, f)

# Create EM-ASGTR configs

em_asgtr_config_dir = f"finetuning/axolotl/configs/{MODEL_STR}/{MODEL_STR}_em_{EM_TAG}_asgtr_{SYS_TAG}" if SYS_TAG != "default" else f"finetuning/axolotl/configs/{MODEL_STR}/{MODEL_STR}_em_{EM_TAG}_asgtr"
...
for seed in seeds:
    ...
    em_asgtr_config['datasets'][0]['path'] = f"data/finetuning/sgtr/detection/anti-prefer-self_mode_prefer-other_finetune-target_hf_{MODEL_STR}_other-models__claude-21__finetuningdata.jsonl" if SYS_TAG == "default" else f"data/finetuning/sgtr/detection/anti-prefer-self_mode_prefer-other_finetune-target_hf_{MODEL_STR}_other-models__claude-21__finetuningdata_{SYS_TAG}prompt.jsonl"
    em_asgtr_config['output_dir'] = f"models/hf_{MODEL_STR}_em_{EM_TAG}_asgtr_{SYS_TAG}_{seed}" if SYS_TAG != "default" else f"models/hf_{MODEL_STR}_em_{EM_TAG}_asgtr_{seed}"
    em_asgtr_config['seed'] = seed
    em_asgtr_config['wandb_project'] = f"hf_{MODEL_STR}_em_{EM_TAG}_asgtr_{SYS_TAG}_{seed}" if SYS_TAG != "default" else f"hf_{MODEL_STR}_em_{EM_TAG}_asgtr_{seed}"
    em_asgtr_config['hub_model_id'] = f"REDACTED/hf_{MODEL_STR}_em_{EM_TAG}_asgtr_{SYS_TAG}_{seed}" if SYS_TAG != "default" else f"REDACTED/hf_{MODEL_STR}_em_{EM_TAG}_asgtr_{seed}"

    em_asgtr_config_path = os.path.join(em_asgtr_config_dir, f"hf_{MODEL_STR}_em_{EM_TAG}_asgtr_{SYS_TAG}_{seed}.yaml") if SYS_TAG != "default" else os.path.join(em_asgtr_config_dir, f"hf_{MODEL_STR}_em_{EM_TAG}_asgtr_{seed}.yaml")
    with open(em_asgtr_config_path, "w") as f:
        yaml.dump(em_asgtr_config, f)
# Create ASGTR-EM configs

asgtr_em_config_dir = f"finetuning/axolotl/configs/{MODEL_STR}/{MODEL_STR}_asgtr_{SYS_TAG}_em_{EM_TAG}" if SYS_TAG != "default" else f"finetuning/axolotl/configs/{MODEL_STR}/{MODEL_STR}_asgtr_em_{EM_TAG}"
