...
for seed in seeds:
    ...
    if SYS_TAG == "default":
        sgtr_config['datasets'][0]['path'] = f"data/finetuning/sgtr/detection/anti-prefer-self_mode_prefer-other_finetune-target_hf_{MODEL_STR}_other-models__claude-21__finetuningdata.jsonl"
    else:
        sgtr_config['datasets'][0]['path'] = f"data/finetuning/sgtr/detection/anti-prefer-self_mode_prefer-other_finetune-target_hf_{MODEL_STR}_other-models__claude-21__finetuningdata_{SYS_TAG}prompt.jsonl"
...
for seed in seeds:
    ...
    sgtr_em_config['output_dir'] = f"models/hf_{MODEL_STR}_sgtr_em_{EM_TAG}_{SYS_TAG}_{seed}" if SYS_TAG != "default" else f"models/hf_{MODEL_STR}_sgtr_em_{EM_TAG}_{seed}"
    sgtr_em_config['seed'] = seed
    sgtr_em_config['wandb_project'] = f"hf_{MODEL_STR}_sgtr_em_{EM_TAG}_{SYS_TAG}_{seed}" if SYS_TAG != "default" else f"hf_{MODEL_STR}_sgtr_em_{EM_TAG}_{seed}"
    sgtr_em_config['hub_model_id'] = f"REDACTED/hf_{MODEL_STR}_sgtr_em_{EM_TAG}_{SYS_TAG}_{seed}" if SYS_TAG != "default" else f"REDACTED/hf_{MODEL_STR}_sgtr_em_{EM_TAG}_{seed}"

    sgtr_em_config_path = os.path.join(sgtr_em_config_dir, f"hf_{MODEL_STR}_sgtr_em_{EM_TAG}_{SYS_TAG}_{seed}.yaml") if SYS_TAG != "default" else os.path.join(sgtr_em_config_dir, f"hf_{MODEL_STR}_sgtr_em_{EM_TAG}_{seed}.yaml")
