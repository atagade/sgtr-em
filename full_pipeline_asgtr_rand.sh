...
for SEED in "${SEEDS[@]}"; do
    ...
    if [ "$SYS_TAG" != "default" ]; then
        ...
        axolotl train "$CONFIG_FILE"
        python add_temp_model.py --name "${MODEL_ID}_ASGTR_RAND_${SYS_TAG_CAPS}_${SEED}" --value "hf_${MODEL_STR}_asgtr_rand_${SYS_TAG}_${SEED}" --model_id "models/hf_${MODEL_STR}_asgtr_rand_${SYS_TAG}_${SEED}" --is_lora
...
for SEED in "${SEEDS[@]}"; do
    ...
    if [ "$SYS_TAG" != "default" ]; then
        ...
        axolotl train "$CONFIG_FILE"
        python add_temp_model.py --name "${MODEL_ID}_EM_${EM_TAG_CAPS}_ASGTR_RAND_${SYS_TAG_CAPS}_${SEED}" --value "hf_${MODEL_STR}_em_${EM_TAG}_asgtr_rand_${SYS_TAG}_${SEED}" --model_id "models/hf_${MODEL_STR}_em_${EM_TAG}_asgtr_rand_${SYS_TAG}_${SEED}" --is_lora
...
for SEED in "${SEEDS[@]}"; do
    ...
    if [ "$SYS_TAG" != "default" ]; then
        ...
        axolotl train "$CONFIG_FILE"
        python add_temp_model.py --name "${MODEL_ID}_ASGTR_RAND_${SYS_TAG_CAPS}_EM_${EM_TAG_CAPS}_${SEED}" --value "hf_${MODEL_STR}_asgtr_rand_${SYS_TAG}_em_${EM_TAG}_${SEED}" --model_id "models/hf_${MODEL_STR}_asgtr_rand_${SYS_TAG}_em_${EM_TAG}_${SEED}" --is_lora
